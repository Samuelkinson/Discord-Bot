...
export default class BatchTS extends SubCommand{
    constructor(client: CustomClient){
        super(client, {
            name: "batch.travis_scott",
        })
    }
    ...
    async Execute(interaction: ChatInputCommandInteraction){
        ...
        try {
            const data = (await BatchConfig.findOne({ guildID: interaction.guild?.id }))?.ts.find(item => item.id === id);
