import { ChatInputCommandInteraction, Collection, Embed, EmbedBuilder, Events } from "discord.js";
...
export default class CommandHandler extends Event{
    ...
    Execute(interaction: ChatInputCommandInteraction) {
        ...
        //         .setColor("Red")
        //         .setDescription("❌You aren't a dev")
        //     ], ephemeral: true});

        if(command.trusted && !this.client.config.trustedUserIds.includes(interaction.user.id))
            return interaction.reply({embeds: [new EmbedBuilder()
                .setColor("Red")
                .setDescription("❌You ain't trusted")
            ], ephemeral: true});
